...
const loginUser = (request, response) => {
  ...
  pool.query('SELECT id,email,password FROM users WHERE email = $1', [email,password], (error, results) => {
    if (error) {
      throw error
    }
...
async function authenticateUser(data) {
  try {
    ...
    const res = await pool.query(
      'SELECT id,email,password FROM users WHERE email = $1 and password = $2', [email,password]
    );
    return (res.rows[0]);
  } catch (err) {
    return err.stack;
